...
Every AWS API call goes through IAM. Click a button in the console? IAM. Run `aws s3 ls`? IAM. Lambda reads from DynamoDB? IAM. Understanding IAM isn't optional — it's the foundation everything else sits on.

This guide gives you a working IAM setup for a startup. Not enterprise complexity. Not "just use AdministratorAccess." The practical middle ground.
...
│  Can't modify         Full control             identity         │
│  Good defaults        Your specific needs      Hard to audit    │
│                                                                 │
│  Examples:            Examples:                When to use:     │
│  • ReadOnlyAccess     • DeveloperPolicy        • Rarely         │
│  • PowerUserAccess    • CICDDeployRole         • Strict 1:1     │
│  • ViewOnlyAccess     • LambdaOrderProcessor     relationship   │
│                                                                 │
│  START HERE ────────▶ GRADUATE TO ──────────▶ AVOID             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
We'll go deeper on these in Part 15 when we cover multi-account architecture.
