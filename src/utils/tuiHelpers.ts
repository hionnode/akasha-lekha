...
export function buildTagIndex(blog: any[], guides: any[], scripts: any[]): Record<string, number> {
...
export function transformContent(items: any[], type: ContentType): ContentItem[] {
  const mapped = items
    .map((item): ContentItem | null => {
      const readingTime = item.body ? calculateReadingTime(item.body) : undefined;
