...
┌──────────────┐       HTTPS        ┌──────────────────────────────┐
│              │ ──────────────────► │         S3 Bucket            │
│   Browser    │                    │   (Website Hosting Enabled)  │
│              │ ◄────────────────  │                              │
└──────────────┘    HTML/CSS/JS     │   index.html (index doc)     │
                                    │   error.html (error doc)     │
                                    │   assets/                    │
                                    └──────────────────────────────┘
                                              │
                                              │ Bucket Policy
...
:::note
**Directory rename.** In Part 4, we used `terraform/` as the root directory. From this part onward, we use `infra/` to better reflect that the directory manages all infrastructure, not just Terraform files. If you are following along, rename your directory: `mv terraform infra`.
:::
...
| | |
|---|---|
| ❌ **Under** | Creating the S3 bucket through the AWS console. No Terraform, no versioning, no reproducibility. When you need a staging environment, you click through the same 15 screens again and inevitably miss a setting. |
| ✅ **Right** | Terraform-managed S3 bucket with website hosting, versioning enabled, bucket policy scoped to `s3:GetObject`, and all resources tagged. One `terraform apply` recreates the entire setup in any environment. |
| ❌ **Over** | Adding CloudFront, Route53, ACM, WAF, and Lambda@Edge before you have confirmed that basic S3 hosting works. Debug one layer at a time. CloudFront is Part 15. |
| 🤖 **Agent Trap** | Agent generates a bucket policy with `Action: "s3:*"` instead of `Action: "s3:GetObject"`. The agent optimizes for "does the website load?" and wildcard permissions guarantee that. But `s3:*` on a public bucket means anyone on the internet can list, write, and delete your files. checkov catches this with CKV_AWS_70. |
