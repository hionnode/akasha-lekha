...
| | |
|---|---|
| ❌ **Under** | No CI pipeline. Manual testing on laptops. "It works on my machine" is the deployment criteria. A dependency update breaks production at 11 PM because nobody ran the full test suite since last week. |
| ✅ **Right** | Parallel CI with test, lint, build, and scan. Matrix strategy for multi-service repos. Docker layer caching for speed. Security scanning on both filesystem and built images. Preview deploy on PRs. 8-12 minute total runtime. |
| ❌ **Over** | 30-minute CI pipeline with redundant checks, full E2E against a staging database, manual approval gates on every PR, separate workflows for every file path, and a Slack notification for each individual job. |
| 🤖 **Agent Trap** | Agent generates a CI workflow with all jobs running sequentially in a single runner. Install three runtimes, run all tests in order, build all images in order. The agent optimizes for simplicity (one YAML file, linear flow), not for speed. A 10-minute parallel pipeline becomes a 30-minute sequential one. |
