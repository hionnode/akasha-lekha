...
| | |
|---|---|
| ❌ **Under** | Hardcoded credentials in source code. `.env` committed to git. Agents with your personal AWS credentials. You find out credentials leaked when AWS emails you about unusual API activity. |
| ✅ **Right** | AWS Secrets Manager for deployed environments. `.env.example` pattern for local dev. aws-vault for scoped agent credentials. Level 2 sandboxing (directory restriction). gitleaks catching credential patterns in pre-commit. |
| ❌ **Over** | HashiCorp Vault cluster with auto-unseal, dynamic database credentials, and PKI infrastructure for mutual TLS between services. You have one database and two developers. |
| 🤖 **Agent Trap** | Agent writes credentials to config files, log statements, or code comments. It does this because the credential was in its context, and it treats everything in context as "known information" that can be referenced anywhere. gitleaks catches the pattern on commit, but the damage is in the agent's output history. The real fix is never putting real credentials in agent context. Use aws-vault exec to inject credentials as environment variables the agent cannot read from its context. |
