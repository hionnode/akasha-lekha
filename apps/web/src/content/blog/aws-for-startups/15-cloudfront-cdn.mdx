...
│                          USERS                                    │
│                                                                    │
│   Mumbai        London        Virginia        Tokyo                │
│     │              │              │              │                  │
└─────┼──────────────┼──────────────┼──────────────┼──────────────────┘
      │              │              │              │
      ▼              ▼              ▼              ▼
┌──────────────────────────────────────────────────────────────────┐
│                    CLOUDFRONT (450+ edges)                         │
│                                                                    │
│   Mumbai Edge    London Edge   Virginia Edge   Tokyo Edge          │
...
# Origin Access Control — the modern way to connect CloudFront to S3
...
:::note
**Static site vs SPA:** This error response configuration returns `/index.html` for 403/404 errors, which is the pattern for single-page applications (React, Vue, etc.). If you're serving a static site as built in Part 13, you may want to remove this block and let CloudFront return the actual error codes. We include the SPA configuration here because Parts 17-18 deploy frontend frameworks that need client-side routing.
:::
...
:::warning
**Migrating from Public to Private:** In Part 13, we set up the S3 bucket with public access for direct website hosting. Now that CloudFront handles content delivery, we need to lock the bucket down: remove the public bucket policy, re-enable the Block Public Access settings, and add an OAC policy that only allows CloudFront to read from the bucket.
:::
...
| | |
|---|---|
| ❌ **Under** | No CDN. Serving directly from S3 in one region. Users in Mumbai wait 800ms. Users in Tokyo wait 400ms. You are paying the same S3 costs but delivering a worse experience. |
| ✅ **Right** | CloudFront with OAC, `CachingOptimized` policy, Gzip/Brotli compression, custom error pages for SPA routing, ACM certificate for SSL termination at the edge. Under $1/month for static sites under 1TB. |
| ❌ **Over** | Lambda@Edge for A/B testing, WAF rules, geo-restrictions, CloudFront Functions for header manipulation, and a custom cache policy per content type. All of these have legitimate uses. None of them are needed before you have traffic. |
| 🤖 **Agent Trap** | Agent uses deprecated Origin Access Identity (OAI) instead of Origin Access Control (OAC). OAI was deprecated in 2022 but dominates the agent's training data. The tell: look for `aws_cloudfront_origin_access_identity` in the generated Terraform. If it exists, replace it with `aws_cloudfront_origin_access_control`. |
