...
You click "Create bucket" in the AWS console. Fifteen minutes later you click it again with slightly different settings. By Friday, you have four buckets, two with public access you did not intend, and no record of which settings you chose for which. Your agent creates resources even faster: a VPC, three subnets, a security group with port 22 open to 0.0.0.0/0, all in a region you did not ask for. Nothing is documented because the console does not document.

**Time:** About 40 minutes.

**Outcome:** A Terraform project with remote S3 state, DynamoDB locking, a reusable module structure, and your first deployed resource. Plus a `.gitignore` and state management strategy that prevents the two most common Terraform disasters.

## What We're Building

- Remote state backend with S3 and DynamoDB locking
- Terraform project structure with environment separation
- Budget alerts codified as infrastructure
- `.gitignore` with a clear secrets strategy
- First `terraform apply` with real AWS resources
...
| | |
|---|---|
| ❌ **Under** | All infrastructure created manually in the console. No record of what exists or why. State stored locally in `terraform.tfstate` on one developer's laptop. Team members cannot collaborate. |
| ✅ **Right** | Remote state in S3 with DynamoDB locking. Environment separation via workspaces or directory structure. Non-sensitive config in committed `terraform.tfvars`, secrets in gitignored `secret.auto.tfvars`. Modules for reusable patterns. |
| ❌ **Over** | Custom Terraform wrapper scripts. Atlantis or Spacelift before you have a second developer. Terragrunt for a single-environment project. Policy-as-code (Sentinel/OPA) for a 3-person team. |
| 🤖 **Agent Trap** | Agent generates a 200-line Terraform file with every resource inline, no modules, no variables, and hardcoded values for region, account ID, and bucket names. It works on `terraform apply` but is unmaintainable, unreusable, and breaks the moment you need a second environment. |
...
:::tip
**tfvars strategy:** We commit `terraform.tfvars` for non-sensitive environment config (instance sizes, feature flags, region settings). Only `secret.auto.tfvars` is gitignored, for values like database passwords. This way your environment configuration is version-controlled and reviewable in PRs, while secrets stay out of the repo.
:::
...
- No locking, so two people can corrupt state simultaneously.
...
## Step 4: Codify Part 1, CloudTrail and Budgets
...
1. Remote state in S3 with DynamoDB locking is non-negotiable from day one, because recovering from a corrupted local state file means reimporting every resource by hand.
2. Commit `terraform.tfvars` for non-sensitive environment config and gitignore `secret.auto.tfvars` for secrets, so your environment settings are reviewable in PRs.
3. Run `terraform plan` before every `apply` and read the output, because the plan is the only moment you can catch an agent's mistake before it becomes infrastructure.
4. `import` blocks bring existing console-created resources under Terraform management without recreating them, which is how you migrate from clicking to coding.
5. Start with flat files and extract modules only when you copy-paste the same resource block a third time.

## What's Coming

Next in **Part 5: OpenTelemetry Setup**, we deploy observability infrastructure with Terraform. The patterns you learned today (modules, remote state, environment config) carry forward into every Terraform file for the rest of the series.
