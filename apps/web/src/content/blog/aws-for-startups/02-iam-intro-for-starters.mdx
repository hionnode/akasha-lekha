---
title: "IAM: The Key to Everything"
...
Your agent spins up three Lambda functions, an API Gateway, and an S3 bucket. Each resource gets `AdministratorAccess` because "it works." Two weeks later, a leaked `.env` file gives an attacker full access to every service in your account. Not because the key was powerful, but because every role was.

**Time:** About 45 minutes.

**Outcome:** A complete IAM setup with developer groups, least-privilege service roles for Lambda, ECS, and CI/CD, OIDC federation for GitHub Actions, and a Terraform role. No `AdministratorAccess` anywhere.

## What We're Building

- Developer group with scoped permissions (no IAM access, no production writes)
- Lambda execution role with least-privilege policy
- GitHub Actions OIDC federation (no access keys)
- Terraform automation role with MFA requirement
- Tag-based permission boundaries for environment isolation
- Naming convention that enables wildcard scoping
...
| | |
|---|---|
| ❌ **Under** | Everyone shares one IAM user with `AdministratorAccess`. Access keys for everything. Policies attached directly to users. No conditions, no reviews. An agent creates a Lambda role with `*:*` because it is the shortest path to a passing test. |
| ✅ **Right** | Users for humans, roles for services. Permissions through groups. AWS managed policies plus narrow custom policies. OIDC for CI/CD. Tag-based conditions for environment scoping. Access Analyzer for quarterly review. |
| ❌ **Over** | Hand-crafting every policy from scratch. 50-line condition blocks on every statement. Weekly permission audits for a 5-person team. Separate IAM role per developer. Custom IdP integration before you have 10 users. |
| 🤖 **Agent Trap** | Agent attaches `AdministratorAccess` to every role because it eliminates permission errors during development. Tests pass, deployment works, and every service in your account can read every secret, delete every database, and modify every IAM policy. The agent optimized for "no errors" instead of "least privilege." |
...
You've seen how roles get assumed and receive temporary credentials. Now let's look at what happens when those credentials are used, and how IAM decides whether to allow or deny any given request.
...
No `Principal` here. This attaches to the Lambda's role. The role itself is the principal.
...
**Trust policy** (`lambda-trust-policy.json`), which defines who can assume this role:
...
- Trust policy only allows Lambda service, so humans can't assume this role
...
1. Humans get users, everything else gets roles. Internalize this rule and you avoid 80% of IAM mistakes.
2. Attach policies to groups, not users. When the team grows from 3 to 30, you change group membership, not 30 individual policy attachments.
3. OIDC federation for CI/CD means no long-lived credentials to rotate, leak, or forget about in a `.env` file.
4. Start with AWS managed policies and narrow them with Access Analyzer after 90 days of real usage data, not before.
5. Conditions (tags, MFA, IP restrictions) let you grant broad permissions safely, which is how you get developer velocity without security theater.

## What's Coming

Next in **Part 3: Your Local AWS Setup**, we configure the AWS CLI with IAM Identity Center so you never hardcode access keys again. The IAM roles you built today become the identities your local environment assumes.
