...
dist/index.html                     1.42 kB │ gzip:  0.73 kB
dist/assets/main-a1b2c3d4.js     142.87 kB │ gzip: 45.21 kB
dist/assets/main-e5f6g7h8.css      8.34 kB │ gzip:  2.15 kB
...
| | |
|---|---|
| ❌ **Under** | Deploy frontend to S3 and call it done. No performance monitoring, no cache strategy. You find out your site is slow when users stop coming. |
| ✅ **Right** | S3 + CloudFront deployment with content-hash caching, OTel browser SDK at 10% sampling, Web Vitals in SigNoz, alerts on LCP/FID/CLS thresholds. |
| ❌ **Over** | Custom RUM solution, A/B testing infrastructure, performance budgets with automated rollbacks, and a dedicated dashboard engineer, all before you have 100 daily users. |
| 🤖 **Agent Trap** | Agent generates OTel config that sends every event unsampled, sets `TraceIdRatioBasedSampler(1.0)`, and creates spans for every DOM interaction. Your SigNoz ingestion bill doubles and your app adds 200ms of telemetry overhead on every page load. |
