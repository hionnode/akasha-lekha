...
```
● api-bun.service - Bun.js API Service
   ...
   Main PID: 12345 (bun)
     ...
     CGroup: /system.slice/api-bun.service
             └─12345 /home/ec2-user/.bun/bin/bun run src/index.ts
...
| | |
|---|---|
| ❌ **Under** | No instrumentation. `console.log` debugging in production. When something is slow, you add more console.log statements, redeploy, wait for it to happen again, and read unstructured text. Your mean time to resolution is measured in hours. |
| ✅ **Right** | OTel auto-instrumentation for HTTP spans plus manual spans on business-critical paths. Traces flowing to SigNoz. Sampling rate set to 100% in dev, 10-50% in production. Service map showing your API. Latency distribution rendering. |
| ❌ **Over** | Custom spans on every function call, 100% trace sampling in production, custom SigNoz dashboards for 15 metrics before you have any traffic to visualize. You are building an observatory before you have a sky to watch. |
| 🤖 **Agent Trap** | Agent instruments everything with 100% sampling and no sampling configuration. SigNoz storage fills up, your monthly observability cost exceeds your compute cost, and the volume of traces makes finding the one slow request harder, not easier. |
