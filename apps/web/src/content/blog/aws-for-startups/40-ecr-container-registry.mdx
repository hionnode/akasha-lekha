...
| | |
|---|---|
| ❌ **Under** | Using Docker Hub for production images. Rate limits break your deployment pipeline on a busy day. Public repository risk exposes your application code. |
| ✅ **Right** | ECR with lifecycle policies (keep last 10 tagged, expire untagged after 1 day), vulnerability scanning on push, Terraform-managed, immutable tags. |
| ❌ **Over** | Multi-region ECR replication, cross-account pull-through cache, for a single-region application with one deployment target. Adds complexity for a scaling problem you don't have yet. |
| 🤖 **Agent Trap** | Agent creates ECR repository without lifecycle policy. Images accumulate indefinitely, storage costs grow linearly, and you don't notice until a quarterly cost review six months later. |
