...
additional gateway — not justified until traffic exceeds 100GB/month
cross-AZ.
...
# Terraform lock file — DO commit this
!.terraform.lock.hcl
...
| | |
|---|---|
| ❌ **Under** | No conventions. `git add .`, commit messages like "wip" and "fix." Agent commits undifferentiated from human commits. Three months later, a security audit asks who approved the IAM policy with `AdministratorAccess` and nobody knows. |
| ✅ **Right** | Conventional commits enforced by commitlint. Specific file staging. Agent attribution via `Co-Authored-By`. PR template with generation method and agent review checklist. |
| ❌ **Over** | GPG signing required on every agent commit, automated squash policies that destroy commit history, mandatory 3-reviewer approval for any file an agent touched, separate repositories for agent-generated code. |
| 🤖 **Agent Trap** | Agent uses `git add .` because it is the shortest path to a clean working tree. The commit sweeps in `.env` with AWS credentials, a debug log with request payloads, and 12 unrelated files the agent modified while exploring. Your `.gitignore` catches the `.env`, but the debug log with customer data ships to GitHub. |
