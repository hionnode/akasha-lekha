...
✓ tflint: 0 issues
✓ checkov: 14/14 checks passed
✓ OPA policies: 6/6 passed
  ...
  - required_tags: PASS
✓ Infracost estimate: $67.14/month
...
resource "aws_db_instance" "main" {
  ...
  # See note below about manage_master_user_password
...
:::note
**Use RDS-managed passwords.** For consistency with [Part 37](/blog/aws-for-startups/37-secrets-manager), use `manage_master_user_password = true` instead of managing the password manually. See Part 37 for the recommended approach.
:::
...
✓ Infracost estimate: $52.34/month
...
:::warning
**Deprecated configuration.** The `forwarded_values` block is deprecated in favor of cache policies (`cache_policy_id`). For new distributions, use `aws_cloudfront_cache_policy` resources or the managed `CachingOptimized` policy (ID: `658327ea-f89d-4fab-a63d-7e88639e58f6`). See [Part 15](/blog/aws-for-startups/15-cloudfront-cdn) for the cache policy approach.
:::
...
✓ OPA policies: 3/3 passed
  - cloudfront_tls_1_2: PASS
  - cloudfront_waf_attached: PASS
  - s3_oac_only: PASS
✓ Infracost estimate: $7.50/month
...
| | |
|---|---|
| ❌ **Under** | Manual deployment, one `terraform apply` at a time, with no verification between modules. You deploy the VPC, then the ECS service, then realize the security group references a subnet that does not exist because you forgot the data subnet module. Debugging takes longer than deploying. |
| ✅ **Right** | Pipeline-driven phased deployment. Each phase generates, verifies, explains, and applies independently. Cost estimates reviewed before apply. Smoke tests validate the full stack. Total deployment takes 2-3 hours, but your confidence in the result is high. |
| ❌ **Over** | Blue-green full-stack deployment for your first deploy. Canary analysis on a service with zero traffic. Automated rollback triggers on an application that nobody is using yet. These are production-day-100 concerns, not production-day-1 concerns. |
| 🤖 **Agent Trap** | Agent generates a single `main.tf` with all 40+ resources in one file, one module, one apply. The pipeline cannot phase the deployment because there are no module boundaries. The explain summary is 500 lines long and unreadable. The agent's reasoning: "fewer files means simpler." The reality: monolithic Terraform is the infrastructure equivalent of a 5,000-line function. |
