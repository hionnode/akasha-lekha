...
:::note
**Secrets Manager integration.** For consistency with [Part 37](/blog/aws-for-startups/37-secrets-manager), store the Redis auth token in AWS Secrets Manager rather than passing it as a plain Terraform variable. Use `aws_secretsmanager_secret_version` data source to retrieve it at apply time.
:::
...
| | |
|---|---|
| ❌ **Under** | No caching layer. Every request hits RDS for session data, user profiles, and site config. Database connection count grows linearly with traffic. At 200 concurrent users, your db.t3.micro hits its connection limit and starts rejecting requests. |
| ✅ **Right** | ElastiCache Redis (cache.t3.micro, $12/month) in private subnets. Cache-aside pattern for read-heavy data. Redis-backed sessions for multi-instance support. TTL on every key. Graceful fallback when Redis is unavailable. |
| ❌ **Over** | Redis Cluster with 3 shards and 2 replicas per shard ($200+/month). Memcached alongside Redis "for different caching needs." A dedicated cache invalidation microservice. DAX for DynamoDB you are not using. All this for an application with 50 daily active users. |
| 🤖 **Agent Trap** | Agent provisions cache.r6g.large ($250/month) because the prompt said "production." It also enables Redis Cluster mode with `num_node_groups = 3`, creating a sharded cluster that adds operational complexity (hash slot management, cross-node commands) your team does not need. The PR template cost impact section catches the instance sizing. The cluster mode requires explicit justification your traffic does not support. |
