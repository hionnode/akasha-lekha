...
| | |
|---|---|
| ❌ **Under** | Using the managed Node.js runtime with full `aws-sdk` v2 import, a 5MB bundle, 600ms cold starts, and no traces. You are flying blind in serverless. |
| ✅ **Right** | Bun custom runtime with OTel traces, optimized bundle under 500KB for API handlers, ARM64 architecture, IAM scoped per function, DLQ on every async function. |
| ❌ **Over** | Custom Lambda extensions for metrics, Datadog Lambda layer, AWS X-Ray integration alongside OTel, performance tuning before you have any traffic to measure. |
| 🤖 **Agent Trap** | Agent bundles all dependencies without tree-shaking, producing a 4MB artifact when 200KB would suffice. It imports `@opentelemetry/auto-instrumentations-node` (2MB of auto-instrumentation for libraries you do not use) instead of the specific exporter and tracer packages. Bundle size is a number, not a feeling: run `bun build` and check the output before deploying. |
