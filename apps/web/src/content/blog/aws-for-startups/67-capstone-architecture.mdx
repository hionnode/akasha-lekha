...
```
                    ...
                    ▼                    ▼                     ▼
           ┌───────────────┐   ┌────────────────┐   ┌──────────────────┐
           │  S3 (Frontend) │   │  ALB (API)     │   │  API Gateway     │
           │  React SPA     │   │  Port 443      │   │  (WebSocket)     │
           └───────────────┘   └───────┬────────┘   └────────┬─────────┘
                                       │                      │
                              ┌────────┴────────┐    ┌────────┴─────────┐
                              │  ECS Fargate     │    │  Lambda           │
                              │  (API Service)   │    │  (WS Handler)    │
                              │  Bun runtime     │    └────────┬─────────┘
                              └────────┬────────┘             │
                                       │                      │
                    ┌──────────────────┼──────────────────────┘
        ...
        │                                         │
  ┌─────┴──────┐  ┌───────────┐  ┌──────────┐  ┌─┴────────┐
  │ RDS Postgres│  │ElastiCache│  │ S3       │  │ SES      │
  │ (Primary)   │  │ (Redis)   │  │ (Assets) │  │ (Email)  │
  └────────────┘  └───────────┘  └──────────┘  └──────────┘
```
...
2. **Session cache:** Temporary session data and rate limiting counters for the API layer.

WebSocket connection IDs are stored in DynamoDB ([Part 56](/blog/aws-for-startups/56-websockets)), not Redis, because connection management requires persistent storage that survives cache eviction.
...
```
GitHub/GitLab Webhook → ALB → ECS API → Validate → SQS Queue → Lambda Worker → RDS
```
...
```
SQS Queue → Lambda Worker → Compute Metrics → RDS Write → Redis Invalidate → SNS Notification
```
...
```
EventBridge Schedule → Lambda → Aggregate Metrics → RDS Write → S3 Report → SES Email
```
...
- Provider: AWS, region as configured in `environments/*.tfvars`
...
| | |
|---|---|
| ❌ **Under** | Jumping straight into Terraform without a design document. You end up with a collection of resources that technically work but have no coherent plan for scaling, security, or cost. When something breaks, you debug by reading Terraform state files because nobody documented the intended architecture. |
| ✅ **Right** | Full architecture design covering compute, data, events, networking, security, and observability. Every decision documented with a "why," not just a "what." Trade-offs explicit. The AGENT-INSTRUCTIONS.md complete and annotated. Ready to hand to the pipeline in Part 68. |
| ❌ **Over** | Spending three weeks on architecture diagrams, capacity planning spreadsheets, disaster recovery simulations, and multi-region failover designs before writing a single line of Terraform. Your application has zero users and you are planning for a million. |
| 🤖 **Agent Trap** | Agent designs architecture based on patterns from its training data (typically enterprise-scale systems), not your actual requirements. It suggests multi-region active-active, Kubernetes with service mesh, and a data lake, for an application that will serve 100 tenants in its first year. The agent optimizes for "production-grade" without a cost constraint, because cost is not in its objective function. Your verify pipeline catches the cost estimate, but only if you run it against the architecture before generating code. |
...
<ValidationChecklist items={[
  ...
  },
  {
    ...
    tasks: [
      ...
      { text: "ElastiCache Redis for caching, DynamoDB for WebSocket connections", syncKey: "part-67-redis" },
