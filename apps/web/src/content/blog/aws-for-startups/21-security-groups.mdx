...
│  │         Inbound: 80 from 0.0.0.0/0 (redirect only)       │       │
│  └───────────────────────┬──────────────────────────────────┘       │
│                          │                                           │
│                   8080 (app port)                                    │
│                          │                                           │
...
| | |
|---|---|
| ❌ **Under** | One security group for everything, `0.0.0.0/0` on all ports. Your database is accessible from the internet, your app tier has no isolation, and a port scan reveals every service you are running. |
| ✅ **Right** | Three-tier security groups (ALB, app, database). Only the ALB allows `0.0.0.0/0`, and only on ports 80 and 443. App and database groups reference the upstream group by ID. Every rule has a description. |
| ❌ **Over** | Network ACLs layered on top of security groups, with WAF rules at every tier, for an internal application that handles 50 requests per day. Three layers of firewalls when one is sufficient for your threat model. |
| 🤖 **Agent Trap** | Agent creates three separate security groups (correct structure) but sets all ingress to `cidr_blocks = ["0.0.0.0/0"]` instead of `source_security_group_id` (no actual isolation). The groups look right in the Terraform code. They pass `terraform validate`. But `checkov` catches it with CKV_AWS_260 because the database accepts traffic from the entire internet. |
