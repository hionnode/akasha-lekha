...
Your API returns a 500 error. You check the logs. There are no logs because nobody set up logging. You add `console.log` statements, redeploy, and reproduce the error. The log says "undefined." You add more logs, redeploy again. Forty-five minutes later, you discover a missing environment variable that a structured trace would have shown in 30 seconds.

**Time:** About 35 minutes.

**Outcome:** An OpenTelemetry Collector running on a free-tier EC2 instance, a SigNoz observability backend with traces, metrics, and logs, and a verification dashboard that proves telemetry flows end-to-end. Every service you build from Part 6 onward connects here automatically.

:::info
**Why observability before applications?** We deploy monitoring infrastructure first so every service we build from Part 6 onward ships with telemetry from day one. Retrofitting observability into existing services is harder and means you miss the early data that reveals architectural issues before they become production incidents.
:::

## What We're Building

- OpenTelemetry Collector on a free-tier EC2 instance
- SigNoz backend for traces, metrics, and logs
- Terraform modules for the observability stack
- AWS Overview Dashboard for cost and resource visibility
- Verification pipeline to confirm telemetry flows end-to-end
...
By the end of this part, you'll have observability infrastructure ready. The dashboards will be empty, and that's intentional. They'll fill up as we deploy applications throughout the series.
...
| | |
|---|---|
| ❌ **Under** | No observability. `console.log` is the debugging strategy. You find out about errors when users complain on Twitter. Metrics are "how does the site feel when I click around." |
| ✅ **Right** | OpenTelemetry Collector with SigNoz. Structured traces from every service. Basic dashboards for latency, error rate, and throughput. Alerts for the metrics that matter. Deployed before your first application so telemetry is never an afterthought. |
| ❌ **Over** | Datadog with every integration enabled ($500/month for a 3-person team). Custom metrics for every function call. 50 dashboards that nobody checks. APM, RUM, synthetic monitoring, and log analytics before you have users. |
| 🤖 **Agent Trap** | Agent configures the OTel Collector to export to three backends simultaneously "for redundancy" and enables every receiver, processor, and exporter in the documentation. The collector consumes more resources than the application it monitors, and the configuration file is 400 lines that nobody understands. |
...
1. Deploy observability before your first application so you never have to retrofit telemetry into a running service under pressure.
2. OpenTelemetry is vendor-neutral, which means switching from SigNoz to Datadog or Grafana Cloud requires changing one exporter config, not re-instrumenting every service.
3. The OTel Collector runs on a t3.micro (free tier) and handles telemetry from dozens of services before you need to think about scaling it.
4. Structured traces beat `console.log` the same way version control beats saving files as `final_v2_REAL.js`.
5. The AWS Overview Dashboard validates your setup works and gives immediate value: cost visibility, resource inventory, and security posture from day one.

## What's Coming

Next in **Part 6: Git & GitHub Agents**, we set up the developer workflow that ties everything together. The observability foundation you built today means every commit, deploy, and rollback generates telemetry you can trace.
