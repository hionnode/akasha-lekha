...
:::warning
**Plan drift alert.** Using `timestamp()` in `final_snapshot_identifier` means every `terraform plan` shows a change, even when nothing else changed. For daily workflows, use a static suffix like `"shipfast-db-final"`. If you want a timestamped name, pass it as a variable at destroy time: `terraform destroy -var="final_snapshot_suffix=$(date +%Y%m%d)"`.
:::
...
:::note
**Coming in Part 37:** We will move the database password into AWS Secrets Manager with automatic rotation. For now, the password lives in `secret.auto.tfvars` (which is gitignored per [Part 4](/blog/aws-for-startups/04-terraform-fundamentals)) and passed as a variable. This is temporary. Hardcoded credentials, even in gitignored files, are a liability.
:::
...
| | |
|---|---|
| ❌ **Under** | PostgreSQL on EC2 with no backups, no encryption, in a public subnet. You lose data when the instance fails and spend weekends patching. |
| ✅ **Right** | RDS in private subnets, db.t3.micro for dev, db.t3.small for prod with Multi-AZ. Automated backups, encryption at rest, Performance Insights enabled. Custom parameter group for slow query logging. |
| ❌ **Over** | db.r6g.2xlarge with Multi-AZ in every environment, provisioned IOPS (io1), read replicas, cross-region replication, and a dedicated KMS key for a startup with 500 users. $800+/month before you have product-market fit. |
| 🤖 **Agent Trap** | Agent enables Multi-AZ in all environments because "it is production best practice." Multi-AZ doubles instance cost. In dev and staging, it adds $15-280/month per environment with zero benefit. Checkov catches publicly accessible databases. Your cost review in the PR template catches instance over-provisioning. |
...
5. Your database password is still in `secret.auto.tfvars`. That changes in Part 37 when Secrets Manager takes over credential management.
