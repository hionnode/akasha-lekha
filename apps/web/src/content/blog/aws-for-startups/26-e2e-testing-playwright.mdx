...
Running 12 tests using 4 workers
  ...
  ✓  1 [chromium] > auth.spec.ts:8:3 > Authentication > successful login (2.1s)
  ✓  2 [chromium] > auth.spec.ts:17:3 > Authentication > invalid credentials (1.8s)
  ✓  3 [chromium] > auth.spec.ts:25:3 > Authentication > logout (2.4s)
  ✓  4 [firefox] > auth.spec.ts:8:3 > Authentication > successful login (3.2s)
  ✓  5 [firefox] > auth.spec.ts:17:3 > Authentication > invalid credentials (2.7s)
  ✓  6 [firefox] > auth.spec.ts:25:3 > Authentication > logout (3.1s)
  ✓  7 [webkit] > auth.spec.ts:8:3 > Authentication > successful login (2.8s)
  ✓  8 [webkit] > auth.spec.ts:17:3 > Authentication > invalid credentials (2.3s)
  ✓  9 [webkit] > auth.spec.ts:25:3 > Authentication > logout (2.9s)
  ✓ 10 [chromium] > users.spec.ts:14:3 > User Management > create user (2.6s)
  ✓ 11 [chromium] > users.spec.ts:26:3 > User Management > search filters (1.9s)
  ✓ 12 [chromium] > error-states.spec.ts:5:3 > Error States > 404 page (1.2s)
...
| | |
|---|---|
| ❌ **Under** | No E2E tests. Manual testing only. You find out the login page is broken when a user emails you. Or worse, when they tweet about it. |
| ✅ **Right** | Playwright with critical path coverage (auth, core CRUD, error states). Three browsers. Page objects for selectors. Traces captured on failure. CI-ready headless config. Tests cover the 5-10 flows that matter most, not every button on the page. |
| ❌ **Over** | 100% E2E coverage. Visual regression on every component. Screenshot diffing on every page in four breakpoints. 200 E2E tests that take 45 minutes to run. CI is permanently blocked by flaky screenshot comparisons. |
| 🤖 **Agent Trap** | Agent generates E2E tests with hardcoded CSS selectors like `page.click('.btn-primary.mt-4')`. The selectors break on any Tailwind class change or component restructure. The fix is accessible locators (`getByRole`, `getByLabel`) wrapped in page objects. Agent-generated tests need immediate refactoring to this pattern. |
