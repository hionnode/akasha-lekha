...
You grep your machine for `AKIA` and find three matches: one in `.bashrc`, one in a `.env` file you forgot about, and one in your shell history that is also synced to iCloud. You rotate the key. A week later, your agent creates a new project and drops a fresh access key into `~/.aws/credentials` because that is what the AWS docs say to do.

**Time:** About 30 minutes.

**Outcome:** AWS CLI v2 configured with IAM Identity Center (SSO), named profiles for dev/staging/prod, session credentials that expire automatically, and aws-vault for an extra layer of protection. Zero long-lived access keys on your machine.

## What We're Building

- AWS CLI v2 with IAM Identity Center (SSO) authentication
- Named profiles for dev, staging, and production environments
- aws-vault for encrypted credential storage
- Automatic session expiration (no permanent keys)
- Shell integration with profile switching
...
The AWS CLI is your primary interface to AWS outside the console. Version 2 is required because it has SSO support that v1 lacks.
...
If you set up your account following Part 1 of this series, you still need to enable Organizations. Part 1 does not cover that step. Follow the guide step below to enable it before proceeding.
...
:::warning
**Clean up the Part 1 IAM user.** Now that SSO handles authentication, deactivate (or delete) the IAM user you created in [Part 1](/blog/aws-for-startups/01-your-first-60-minutes-in-aws). Having unused credentials is a security risk. In the IAM console, disable the access keys and remove console access for that user.
:::
...
| | |
|---|---|
| ❌ **Under** | One set of access keys in `~/.aws/credentials`. Same keys for dev and production. Keys never rotated. The agent uses whatever credentials it finds, which happen to have admin access to production. |
| ✅ **Right** | IAM Identity Center with named profiles. Session credentials that expire in 1-12 hours. aws-vault for encrypted storage. Separate profiles per environment so `--profile prod` is a deliberate choice. |
| ❌ **Over** | Hardware security keys required for every CLI session. Custom credential broker with audit logging for a solo developer. Separate laptops for production access. |
| 🤖 **Agent Trap** | Agent runs `aws configure` and stores a permanent access key in plaintext because it is the fastest path to a working CLI. The key has admin permissions "for development." Six months later, nobody remembers the key exists, it has never been rotated, and it has full access to production. |
...
If `[default]` is configured, commands without `--profile` use it. But explicit is better than implicit, so use `AWS_PROFILE` or `--profile`.
...
1. IAM Identity Center replaces long-lived access keys with session credentials that expire automatically, which eliminates the entire category of "leaked permanent key" incidents.
2. Named profiles force you to choose an environment explicitly, which means deploying to production requires typing `--profile prod` instead of accidentally using your default credentials.
3. aws-vault encrypts credentials at rest so a stolen laptop does not mean stolen AWS access.
4. The `AKIA` grep test should return zero results on your machine. If it does not, you have permanent credentials that need to be replaced with SSO sessions.

## What's Coming

Next in **Part 4: Terraform Fundamentals**, we codify everything in infrastructure-as-code. The SSO profiles you configured today become the authentication method Terraform uses to manage your AWS resources.
