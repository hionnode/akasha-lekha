...
## The Simple Path: RDS-Managed Passwords

Update the RDS module from [Part 35](/blog/aws-for-startups/35-rds-postgres) to let RDS handle the password entirely:
...
RDS has a native Secrets Manager integration via `manage_master_user_password`. When set to `true`, RDS creates a Secrets Manager secret for the master password automatically and handles rotation. No Lambda function, no IAM roles for rotation, no security group rules. One line of Terraform and the password is managed, rotated, and stored in Secrets Manager.

You can retrieve the ARN of the auto-managed secret from the RDS instance attributes:
...
For most applications, RDS-managed passwords are sufficient. They handle creation, storage, encryption, and rotation with zero custom infrastructure. If you are setting up a new RDS instance, start here and move on to the application integration section below.

For existing instances (like the one from Part 35) or cases where you need custom rotation logic (multi-user rotation, cross-account secrets, non-RDS credentials), manual Secrets Manager rotation gives you full control. That approach follows after the application integration section.
...
## Advanced: Custom Rotation with Lambda

For most startups, `manage_master_user_password = true` is the right answer. But there are legitimate reasons to manage rotation yourself: you need multi-user rotation strategies, you are rotating non-RDS credentials (API keys, Redis AUTH tokens), you need cross-account secret sharing, or you want custom validation logic during rotation. This section covers the full manual setup.
...
For existing instances (like the one from Part 35), the migration to manual rotation requires:
...
| | |
|---|---|
| ❌ **Under** | Database password hardcoded in terraform.tfvars (or worse, in application code). Never rotated. The password from initial setup is still active 18 months later on every developer's laptop. |
| ✅ **Right** | Secrets Manager with automatic 30-day rotation for RDS credentials. Application retrieves credentials at startup with in-memory cache and retry-on-failure. Rotation Lambda in VPC with scoped IAM permissions. |
| ❌ **Over** | HashiCorp Vault cluster with dynamic database credentials, Vault Agent sidecar on every container, and a dedicated Vault ops team for a single PostgreSQL instance. $500+/month in infrastructure plus ongoing operational overhead. |
| 🤖 **Agent Trap** | Agent stores the database password directly in the `aws_db_instance` resource's `password` field, which means the password appears in plaintext in Terraform state. The agent does this because it is the shortest path to a working `terraform apply`. Checkov flags unencrypted secrets in state. The fix: use `manage_master_user_password = true` or reference a Secrets Manager ARN. |
