...
| | |
|---|---|
| ❌ **Under** | Lambda function URLs with no throttling, no routing, and no custom domain. Each function has its own URL. A traffic spike exhausts your concurrency limit and costs $500 before you notice. |
| ✅ **Right** | HTTP API Gateway with routes, 1,000 rps throttling, custom domain, JSON access logs, CORS configured. Each environment has its own stage. |
| ❌ **Over** | REST API with VTL request/response transformations, API keys with usage plans, built-in caching, WAF integration, and request model validation for a pre-launch API that handles 50 requests per day. That is $3.50/million requests instead of $1.00/million, plus the cognitive cost of VTL templates. |
| 🤖 **Agent Trap** | Agent creates `aws_api_gateway_rest_api` (REST API) instead of `aws_apigatewayv2_api` with `protocol_type = "HTTP"` (HTTP API). REST API has more examples in training data but costs 3.5x more per request and adds features you do not need. The Terraform resource names are confusing: v1 resources (`aws_api_gateway_*`) are REST API, v2 resources (`aws_apigatewayv2_*`) are HTTP API and WebSocket. Always use v2 unless you need REST-specific features. |
