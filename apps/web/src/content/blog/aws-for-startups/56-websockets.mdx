...
:::note
**Replacing the earlier definition.** This DynamoDB table configuration supersedes the basic version shown above. In your actual Terraform code, use only this final version.
:::
...
| | |
|---|---|
| ❌ **Under** | Polling every 5 seconds for updates. Your frontend makes 12 API requests per minute per user. At 1,000 concurrent users, that is 12,000 requests per minute, most returning "no updates." Your API Gateway bill and Lambda invocations scale with users, not with actual events. |
| ✅ **Right** | API Gateway WebSocket API with Lambda handlers for connect, disconnect, and default routes. Connection IDs in DynamoDB with TTL. Token-based auth on `$connect`. GoneException handling for stale connections. Keep-alive pings every 5 minutes. |
| ❌ **Over** | Self-managed WebSocket servers on ECS with custom auto-scaling, Redis pub/sub for cross-instance message distribution, and a dedicated connection management service. You are building Slack's infrastructure for an app with 100 concurrent users. |
| 🤖 **Agent Trap** | Agent stores connection IDs in a module-level Map or array in the Lambda handler. This works exactly once. On the next invocation (different container) or after a cold start, the in-memory store is empty, and all connected clients become unreachable. The fix is always DynamoDB. Code review catches any module-level mutable state in Lambda handlers. |
